...
def test_generate_key_and_shares_threshold():
    threshold = 2
    num_of_shares = 5
    _, key_shares, _ = generate_key_and_shares(threshold, num_of_shares)
...
def test_encrypt_and_partial_decrypt_and_combine():
    ...
    pub_key, key_shares, t_params = generate_key_and_shares(threshold, num_of_shares)
    ...
    recovered = decrypt([partial_1, partial_2], encrypted, t_params)
...
def test_partial_decrypt_invalid_share():
    ...
    num_of_shares = 3
    message = "test"
    pub_key, _, _ = generate_key_and_shares(threshold, num_of_shares)
    encrypted = encrypt(message, pub_key)
...
def test_combine_partial_decryptions_insufficient_shares():
    ...
    with pytest.raises(Exception):
        decrypt(partials, encrypted, t_params)
