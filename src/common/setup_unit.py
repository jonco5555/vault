...
from common.constants import SETUP_PORT, SETUP_MASTER_DNS_ADDRESS
...
class SetupUnit(setup_pb2_grpc.SetupUnit):
    def __init__(
        ...
        self._running_server_task = asyncio.create_task(self._start_setup_unit_server(self._server_ip))
    ...
    async def _start_setup_unit_server(self, server_ip: str = "[::]"):
        try:
            self._running_server = grpc.aio.server(futures.ThreadPoolExecutor(max_workers=2))
            ...
            print(f"SetupUnit started start_setup_master_server on port {SETUP_PORT}...")
